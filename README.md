...
- **`getNextTask`**:
  - **Description:** Identifies the next actionable executable task based on status ('todo'), terminal dependencies ('done' or 'cancelled'), dependency-graph impact, estimated complexity, priority, and creation date. If `sprint_id` is omitted, the most recent active sprint is used when one exists.
  - **Params:** `project_id` (string, required, UUID), `sprint_id` (string, optional, UUID), `limit` (integer, optional, 1-10; use 3 for the three best candidates), `max_complexity` (enum 'low'|'medium'|'high', optional), `include_explanations` (boolean, optional)
  - **Returns:** By default, a `FullTaskData` object of the next task, or `null` if none are ready. When `limit`, `max_complexity`, or `include_explanations` is supplied, returns a ranked selection with `candidates`, scores, reasons, unblock data, dependency depth, and blocked graph summary.
- **`createSprint`**:
...
- **`importProject`**:
  - **Description:** Creates a _new_ project from an exported JSON string.
