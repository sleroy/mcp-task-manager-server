...
import {
  ...
  ValidationError,
  ConflictError,
} from "../utils/errors.js"; // Import errors
...
export class ProjectService {
  ...
  public async createProject(projectName?: string): Promise<ProjectData> {
    ...
    logger.info(
      `[ProjectService] Attempting to create project: ${projectId} with name "${finalProjectName}"`
    );
    try {
      this.projectRepository.create(newProject);
      logger.info(
        `[ProjectService] Successfully created project: ${projectId}`
      );
      return newProject;
    } catch (error) {
      logger.error(
        `[ProjectService] Error creating project ${projectId}:`,
        error
      );
      throw error;
    }
  ...
  public async getProjectById(
    ...
    try {
      ...
      logger.error(
        `[ProjectService] Error finding project ${projectId}:`,
        error
      );
      throw error;
    }
  ...
  public async exportProject(projectId: string): Promise<string> {
    ...
    if (!projectMetadata) {
      logger.warn(
        `[ProjectService] Project not found for export: ${projectId}`
      );
    ...
    try {
      ...
      logger.info(
        `[ProjectService] Successfully prepared export data for project ${projectId}`
      );
      return jsonString;
    } catch (error) {
      logger.error(
        `[ProjectService] Error exporting project ${projectId}:`,
        error
      );
      throw error;
    }
  ...
  public async exportProjectSnapshot(
    projectId: string,
    outputPath: string
  ): Promise<{ project_id: string; output_path: string; bytes: number }> {
  ...
  public async importProject(
    projectDataString: string,
    newProjectName?: string
  ): Promise<{ project_id: string }> {
    logger.info(`[ProjectService] Attempting to import project...`);
    let importData: ExportData;
    try {
      if (projectDataString.length > 10 * 1024 * 1024) {
        // Example 10MB limit
        throw new ValidationError(
          "Input data exceeds size limit (e.g., 10MB)."
        );
      }
      ...
      if (
        !importData ||
        !importData.project_metadata ||
        !Array.isArray(importData.tasks)
      ) {
        throw new ValidationError(
          "Invalid import data structure: Missing required fields."
        );
      }
      logger.debug(`[ProjectService] Successfully parsed import data.`);
    } catch (error) {
      logger.error(
        "[ProjectService] Failed to parse or validate import JSON:",
        error
      );
      ...
      throw new ValidationError(
        `Invalid import data: ${error instanceof Error ? error.message : "Unknown validation error"}`
      );
    }
    ...
    const importTransaction = this.db.transaction(() => {
      ...
      const finalProjectName =
        newProjectName?.trim() ||
        `${importData.project_metadata.name} (Imported ${now})`;
      const newProject: ProjectData = {
      ...
      this.projectRepository.create(newProject);
      logger.info(
        `[ProjectService] Created new project ${newProjectId} for import.`
      );
      ...
      const processTask = (task: ExportTask, parentDbId: string | null) => {
        ...
        const newTaskData: TaskData = {
          ...
          sprint_id: task.sprint_id
            ? (sprintIdMap.get(task.sprint_id) ?? null)
            : null,
        ...
        if (newTaskId && task.dependencies && task.dependencies.length > 0) {
          for (const oldDepId of task.dependencies) {
            ...
            if (newDepId) {
              ...
              logger.warn(
                `[ProjectService] Dependency task ID ${oldDepId} not found in import map for task ${task.task_id}. Skipping dependency.`
              );
            }
          }
        }
      ...
      logger.info(
        `[ProjectService] Processed ${depCount} dependencies for import.`
      );
    ...
    try {
      ...
      logger.info(
        `[ProjectService] Successfully imported project. New project ID: ${result.project_id}`
      );
      return result;
    } catch (error) {
      ...
      throw new Error(
        `Failed to import project: ${error instanceof Error ? error.message : "Unknown database error"}`
      );
    }
  }
  ...
  public async deleteProject(projectId: string): Promise<boolean> {
    ...
    if (!projectExists) {
      logger.warn(
        `[ProjectService] Project not found for deletion: ${projectId}`
      );
      throw new NotFoundError(`Project with ID ${projectId} not found.`);
    }
    ...
    try {
      ...
      if (deletedCount !== 1) {
        ...
        logger.warn(
          `[ProjectService] Expected to delete 1 project, but repository reported ${deletedCount} deletions for project ${projectId}.`
        );
        // Still return true as the project is gone, but log indicates potential issue.
      }

      logger.info(
        `[ProjectService] Successfully deleted project ${projectId} and associated data.`
      );
