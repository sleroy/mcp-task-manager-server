...
export class TaskRepository {
    ...
    const transaction = this.db.transaction(
      (taskData: TaskData, deps: string[]) => {
        ...
        if (taskInfo.changes !== 1) {
          throw new Error(
            `Failed to insert task ${taskData.task_id}. Changes: ${taskInfo.changes}`
          );
        }
  ...
  public findByProjectId(
    ...
    try {
      ...
      logger.debug(
        `[TaskRepository] Found ${tasks.length} tasks for project ${projectId}`,
        filters
      );
      return tasks;
    } catch (error) {
  ...
  public findById(projectId: string, taskId: string): TaskData | undefined {
    ...
    try {
      ...
      logger.debug(
        `[TaskRepository] Found task ${taskId} in project ${projectId}: ${!!task}`
      );
      return task;
    } catch (error) {
      logger.error(
  ...
  /**
   * Finds tasks that are ready to be worked on (status 'todo' and all dependencies terminal).
   ...
   */
  public findReadyTasks(
    projectId: string,
    sprintId?: string | null
  ): TaskData[] {
    // This query finds tasks in the project with status 'todo'
    // AND for which no dependency exists OR all existing dependencies are terminal.
    let sql = `
            ...
            AND NOT EXISTS (
                ...
                WHERE td.task_id = t.task_id AND dep_task.status NOT IN ('done', 'cancelled')
            )
        `;
    const params: (string | null)[] = [projectId];
  ...
  public findDescendants(projectId: string, taskId: string): TaskData[] {
    ...
    try {
      const descendants = this.db
        .prepare(sql)
        .all(projectId, taskId, projectId) as TaskData[];
      logger.debug(
        `[TaskRepository] Found ${descendants.length} descendants for task ${taskId}`
      );
      return descendants;
    } catch (error) {
      logger.error(
        `[TaskRepository] Failed to find descendants for task ${taskId}:`,
        error
      );
      throw error;
    }
  ...
  public updateSprintStatus(
    ...
    try {
      const info = this.db
        .prepare(sql)
        .run(status, timestamp, projectId, sprintId);
      logger.info(
        `[TaskRepository] Updated ${info.changes} sprint tasks to ${status} for sprint ${sprintId}.`
      );
      return info.changes;
    } catch (error) {
      logger.error(
        `[TaskRepository] Failed to update sprint ${sprintId} tasks:`,
        error
      );
      throw error;
    }
  ...
  public updateSprintAssignment(
    ...
    try {
      const info = this.db
        .prepare(sql)
        .run(sprintId, timestamp, projectId, ...taskIds);
      logger.info(
        `[TaskRepository] Updated sprint assignment for ${info.changes} tasks in project ${projectId}.`
      );
      return info.changes;
    } catch (error) {
      logger.error(
        `[TaskRepository] Failed to update sprint assignments in project ${projectId}:`,
        error
      );
      throw error;
    }
  ...
  public updateTask(
    ...
    const transaction = this.db.transaction(() => {
      ...
      if (setClauses.length > 0) {
        ...
        if (changes !== 1) {
          ...
          if (!exists) {
            ...
            ); // Will be caught and mapped later
          } else {
            throw new Error(
              `Failed to update task ${taskId}. Expected 1 change, got ${changes}.`
            );
          }
        }
      ...
      if (updatePayload.dependencies !== undefined) {
        ...
        logger.debug(
          `[TaskRepository] Deleted ${deleteInfo.changes} existing dependencies for task ${taskId}.`
        );
        ...
        logger.debug(
          `[TaskRepository] Inserted ${newDeps.length} new dependencies for task ${taskId}.`
        );
      }
      ...
      if (!updatedTask) {
        ...
        throw new Error(
          `Failed to retrieve updated task ${taskId} after update.`
        );
      }
      return updatedTask;
    });
