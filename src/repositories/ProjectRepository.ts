...
export class ProjectRepository {
  ...
  public create(project: ProjectData): void {
    ...
    try {
      ...
      logger.error(
        `[ProjectRepository] Failed to create project ${project.project_id}:`,
        error
      );
      if (error instanceof SqliteError) {
        ...
        if (error.code === "SQLITE_CONSTRAINT_PRIMARYKEY") {
          throw new RepositoryConflictError(
            `Project with ID '${project.project_id}' already exists.`
          );
        }
      }
  ...
  public findById(projectId: string): ProjectData | undefined {
    ...
    try {
      ...
      logger.error(
        `[ProjectRepository] Failed to find project ${projectId}:`,
        error
      );
    ...
    try {
      ...
      if (info.changes === 0) {
        throw new RepositoryNotFoundError(
          `Project with id ${projectId} not found, cannot delete.`
        );
      }
      logger.info(
        `[ProjectRepository] Deleted project ${projectId}. Rows affected: ${info.changes}`
      );
      // Cascade delete handles tasks/dependencies in the background via schema definition.
      return info.changes;
    } catch (error) {
      logger.error(
        `[ProjectRepository] Failed to delete project ${projectId}:`,
        error
      );
   ...
   * @returns An array of all project records.
   */
  public findAll(): ProjectData[] {
    ...
    try {
      ...
      logger.error(`[ProjectRepository] Failed to list all projects:`, error);
      throw error;
    }
  }
  ...
  public update(projectId: string, projectName: string): { updated: number } {
    logger.debug(
      ...
      "Attempting to update project name in DB"
    );
    ...
    if (info.changes === 0) {
      ...
      throw new RepositoryNotFoundError(
        `Project with id ${projectId} not found, or project name is already set to the provided value.`
      );
    }
    logger.info(
      { projectId, newName: projectName, changes: info.changes },
      "Project name updated successfully in DB"
    );
