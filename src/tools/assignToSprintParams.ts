...
export const TOOL_PARAMS = z.object({
    ...
    task_ids: z.array(z.string().uuid("Each task ID must be a valid UUID.")).min(1).max(100).optional()
        .describe("Explicit stories/tasks to assign. Optional when using selectors."),
    ...
    item_type: WorkItemTypeEnum.optional()
        .describe("Optional selector for stories or tasks. Epics are never assigned directly."),
    status: TaskStatusEnum.optional()
        .describe("Optional selector for work item status."),
    parent_task_id: z.string().uuid("The parent_task_id must be a valid UUID.").nullable().optional()
        .describe("Optional selector for direct children of a parent, or null for root work items."),
    epic_id: z.string().uuid("The epic_id must be a valid UUID.").optional()
        .describe("Optional selector that assigns matching descendant stories/tasks of an epic."),
    milestone: z.string().min(1).max(128).nullable().optional()
        .describe("Optional selector for a lightweight milestone label. If omitted and the target sprint has a milestone, that sprint milestone is used."),
